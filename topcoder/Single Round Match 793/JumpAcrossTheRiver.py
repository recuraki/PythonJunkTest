...
def do_test(N, M, J, __expected):
    startTime = time.time()
    instance = JumpAcrossTheRiver()
    exception = None
    try:
        __result = instance.minLength(N, M, J);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("JumpAcrossTheRiver (500 Points)\n\n")
    ...
    with open("JumpAcrossTheRiver.sample", "r") as f:
        while True:
            label = f.readline()
            ...
            passed += do_test(N, M, J, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1604542409
    PT, TT = (T / 60.0, 75.0)
