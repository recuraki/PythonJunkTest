...
def do_test(S, __expected):
    startTime = time.time()
    instance = SmallestRegular()
    exception = None
    try:
        __result = instance.findLexSmallest(S);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("SmallestRegular (500 Points)\n\n")
    ...
    with open("SmallestRegular.sample", "r") as f:
        while True:
            label = f.readline()
