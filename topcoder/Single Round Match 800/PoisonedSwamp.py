        ...
        for hh in range(h):
            for ww in range(w):
                if maze[hh][ww] == "#":
                    continue
                if maze[hh][ww] == "S":
                    anotherGoal.append(map2floyd(hh, ww))
                ...
                for di in range(len(dw)):
                    ...
                    if maze[nh][nw] == "#":
                        continue
                    dj.makeEdge(map2floyd(nh, nw), map2floyd(hh, ww), curCost)
        dj.solve(0,0)
        #print(dj.cost)
        #print(anotherGoal)

        if dj.cost[map2floyd(h-1, w-1)] == dj.INF:
            return "impossible"

        for x in anotherGoal:
            if dj.cost[x] < 10:
                return "possible"
        return "impossible"
