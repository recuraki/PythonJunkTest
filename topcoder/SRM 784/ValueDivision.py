...
def do_test(A, __expected):
    startTime = time.time()
    instance = ValueDivision()
    exception = None
    try:
        __result = instance.getArray(A);
    except:
        import traceback
...
def run_tests():
    sys.stdout.write("ValueDivision (1000 Points)\n\n")
    ...
    with open("ValueDivision.sample", "r") as f:
        while True:
            label = f.readline()
